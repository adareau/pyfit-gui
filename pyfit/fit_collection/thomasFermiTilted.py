...
import thomasFermi2D as TF2D

#p = [offset, Ampl,sx,sy,cx,cy,theta]

def ThomasFermiTilted((x,y),*p):
    ...
    return TF2D.ThomasFermi2D((xr,yr),*pnew)
...
def gen():
    ...
    fit = TF2D.gen()
    fit.formula = lambda(x,y),*p:ThomasFermiTilted((x,y),*p)
    fit.name = 'ThomasFermiTilted'
    fit.guess = guess
