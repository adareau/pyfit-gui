...
class StartQT4(QtGui.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        scrollWidget.setWidget(self.ui.centralwidget)
        ...
        qd = QtCore.QDir
        self.folder_tree_model.setFilter(qd.Dirs | qd.Drives | qd.NoDotAndDotDot | qd.AllDirs)
        ...
        for i in [1,2,3]:self.ui.folder_tree.hideColumn(i)
        ...
        #XXX screen
        '''
        self.ui.plotWindow.main_axes.imshow(start_image,extent=(0, image_size_x, 0, image_size_x),cmap=plt.get_cmap(self.settings.colormap))
        ...
        self.ui.fit_binning_maxpoints_txt.textEdited.connect(self.refresh_fit_settings)
        self.ui.fit_binning_maxpoints_txt.setValidator(QtGui.QIntValidator(1,2000))        
        self.ui.fit_binning_txt.textEdited.connect(self.refresh_fit_settings)
        self.ui.fit_binning_txt.setValidator(QtGui.QIntValidator(1,1000))
        
        for f in self.data.fit2D_dic:
            self.ui.fit_type.addItem(f)
        ...
        self.ui.refresh_display_button.clicked.connect(lambda :self.display_file(True)) # a bit cumbersome, but we have to do that to set load_fit=True to display_data
        ...
        self.ui.display_interpolation.currentIndexChanged.connect(self.display_interpolation_clicked)
        ...
        for f in self.data.fit1D_dic:
            self.ui.list_fit_type.addItem(f)
        ...
        ns = {'gui': self,'plt':plt,'np':np,'fit':self.fit()}
        self.pythonshell = internalshell.InternalShell(self.ui.console_dock,namespace=ns,commands=['gui.init_shell()'],multithreaded=False)
    ...
    def update_file_list(self):
        ...
        qd = QtCore.QDir
        currentDir = qd(path)
        files_list = currentDir.entryList(file_name_filter,qd.Files | qd.NoSymLinks,QtCore.QDir.Time) 
        ...
        for f in files_list:
            ...
            if os.path.isfile(fit_path):
                name = self.settings.isfit_str+str(f)
            else:
                name = self.settings.isnofit_str+str(f)
        ...
        self.ui.file_list.selectionModel().selectionChanged.connect(self.file_list_clicked) # has to be set after model is set for file_list
        #print(file_list)
    ...
    def draw_ROI(self,draw=True):
        ...
        if self.data.rectROI is not None:
            patches = self.ui.plotWindow.main_axes.patches
            if self.data.rectROI in patches:
                self.data.rectROI.remove()
            
        self.data.rectROI = ROI = Rectangle((r[0],r[2]), r[1]-r[0],r[3]-r[2] ,alpha=1,fc='none',ec='green',linewidth=2)
        
        self.ui.plotWindow.main_axes.add_patch(ROI)

        
        self.data.ed_rectROI = pf.EditableRectangle.EditableRectangle(ROI,fixed_aspect_ratio=False)
        if self.ui.toolbar_ROI.isChecked():
            self.data.ed_rectROI.connect()
    ...
    def draw_background(self, draw=True):
        ...
        self.data.rectBackground = background = Rectangle((r[0],r[2]), r[1]-r[0],r[3]-r[2] ,alpha=1,fc='none',ec='red',linewidth=2)
        
        self.ui.plotWindow.main_axes.add_patch(background)

        
        self.data.ed_rectBackground = pf.EditableRectangle.EditableRectangle(background,fixed_aspect_ratio=False)
        if self.ui.toolbar_background.isChecked():
            self.data.ed_rectBackground.connect()
    ...
    def get_background(self): 
        ...
        if BCK is not None:
            r = (BCK.xy[0],BCK.xy[0]+BCK.get_width(),BCK.xy[1],BCK.xy[1]+BCK.get_height())
    ...
    def plot_fit_results(self,fitObj=None):
        ...
        if self.settings.display_fit_contour:
            ...
            self.ui.plotWindow.main_axes.contour(xm,ym,fit_res,8,colors='w')
            #self.ui.plotWindow.draw()
    ...
    def refresh_fit_settings(self,event):
        ...
        if load:
            ...
            if self.settings.current_fit_type != self.ui.fit_type.currentText():
                self.settings.current_fit_type = str(self.ui.fit_type.currentText())
                self.data.current_fit.fit = self.data.fit2D_dic[self.settings.current_fit_type]
                self.data.current_fit.fit.options = self.settings.current_fit_options
    ...
    def refresh_gui_settings(self,event):
        ...
        if load:
            ...
            index = self.ui.display_interpolation.findText(self.settings.display_interpolation)
            if index > -1:
    ...
    def add_new_cam(self):
        ...
        camera_name,ok = QtGui.QInputDialog.getText(self,'PyF!T wanna know...',
    ...
    def delete_cam(self):
        ...
        reply = QtGui.QMessageBox.question(self,'PyF!T wanna know...',
                                           'Delete this camera ?', QtGui.QMessageBox.Yes,
...
class GuiData():
    ...
    def __init__(self,gui):
        ...
        self.colormaps = ['jet','binary','hot','gray','Blues','Greens','rainbow']
        self.interpolations = ['none','bilinear','bicubic','gaussian','hermite']
